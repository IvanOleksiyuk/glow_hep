...
class Invertible1x1conv(tfb.Bijector):
    def __init__(self, input_shape, name = "Invertible1x1conv", **kwargs):
    ...
    def build(self):
        ...
        p_mat = tf.Variable(name="P_mat", initial_value=np_p, trainable=False)
        l_mat = tf.Variable(name="L_mat", initial_value=np_l)
        sign_s = tf.Variable(name="sign_S", initial_value=np_sign_s, trainable=False)
        log_s = tf.Variable(name="log_S", initial_value=np_log_s)
        u_mat = tf.Variable(name="U_mat", initial_value=np_u)
        ...
        u_mat = u_mat * np.transpose(l_mask) + tf.linalg.diag(sign_s * tf.exp(log_s))
        w = tf.matmul(p_mat, tf.matmul(l_mat, u_mat))

        # inverse w
        u_inv = tf.linalg.inv(u_mat)
        l_inv = tf.linalg.inv(l_mat)
        p_inv = tf.linalg.inv(p_mat)
